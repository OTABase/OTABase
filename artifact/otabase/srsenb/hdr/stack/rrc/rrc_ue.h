...
public:
  ...
  enum activity_timeout_type_t {
    ...
    OTAFUZZ_REATTACH_TIMEOUT,  ///< When UE is not reconnected after this timer, send paging. 
    nulltype
  };
  ...
  void send_ue_info_req();
   ...
   /* OTAFUZZ */
  void send_nas_test_message();
  void send_rrc_test_message();
  bool allow_next_msg = false;
  std::string rand_hex_stream(int length);
  void send_connection_release_tau();
  void send_connection_release_other();
  void send_paging_message();
  bool ready_for_test = false;
  bool waiting_for_nas_oracle = false;
  int  socketfd = -1;
  bool fuzz;
  pthread_mutex_t socket_mutex;
  int  num_sent_msg = 0;
  int  num_msg_N_oracle = 0;
  bool set_reattach_timeout();
  bool is_reestab_procedure = false;
  bool oracle_enabled = true;
  void notify_ack_timeout();

  bool is_doing_state_transition = false;
  // temporary bools for sending test message
  bool read_from_file = false;
  bool read_from_parent_file = true;

  // temporary bools for handling the oracle
  bool is_using_python = false;
  bool waiting_for_rrc_oracle = false;
  ...
  bool send_dl_dcch_bytes(std::string                     payload, 
                          srsran::unique_byte_buffer_t    pdu       = srsran::unique_byte_buffer_t());
