...
#define TEST_NAS 1
#define NO_EVAL 0
#define EVAL_IDLE_TIME 1
#define EVAL_PROCESSING_SPEED 2
...
public:
  ...
  void send_next_test_msg(uint16_t rnti) override;

  // rrc_interface_s1ap
  void     write_dl_info(uint16_t rnti, srsran::unique_byte_buffer_t sdu, bool is_id_req) override;
  ...
  /* OTAFUZZ */
  void     send_paging(uint64_t imsi);
  int      total_sent_msg = 0;
  uint32_t prev_tmsi = 0;
  uint32_t cur_tmsi = 0; 
  uint16_t recent_rnti = 0;
  void send_paging(uint64_t imsi, uint32_t tmsi);
  bool set_reattach_timer();
  bool stop_reattach_timer();
  void reattach_timer_expired(uint32_t timeout_id);

  bool set_nas_oracle_timer();
  void nas_oracle_timer_expired(uint32_t timeout_id);

  bool set_rrc_oracle_timer();
  void rrc_oracle_timer_expired(uint32_t timeout_id);
  uint16_t rrc_oracle_cnt = 0;

  int  paging_attempt = 0;
  int  airplane_mode_timer = 120;

  void toggle_airplane_mode(const std::string& device_name);
  int  airplanemode_attempt = 0;
  ...
  void     send_connection_release_tau(uint16_t rnti) override;
  void     send_connection_release_other(uint16_t rnti) override;
  bool     set_ready_for_test(uint16_t rnti, bool is_ready_for_test) override;
  bool     set_reattach_timeout(uint16_t rnti, bool run_the_timer) override;
  ...
  uint16_t      backtracking_num_total = 0;
...
private:
  ...
  const static uint64_t delay_command = 900000; // 0.9s

  static int executeShellCommand(const char* command);
  ...
  // Used for reconnecting the UE
  srsran::unique_timer repaging_timer; 

  // Used for checking the NAS oracle
  srsran::unique_timer nas_oracle_timer; 

  // Used for checking the RRC oracle
  srsran::unique_timer rrc_oracle_timer; 
  ...
  // Used for blacklisting the consecutive mutations. 
  void blacklist_test_cases(std::string& blacklist_msgName_path);

  // Used for temporal blacklisting, blackinglisting certain IE for a while.
  void temp_blacklist_test_cases(std::string& blacklist_msgName_path);
  ...
  // Used for temporal blacklisting, blackinglisting certain IE for a while. 
  std::unordered_map<std::string, int> blacklistMsgFieldCount;
  std::vector<std::string>             blacklistMsgField;
  const int maxCount = 3; // Maximum count before blacklisting
  const int resetThreshold = 30; // Number of lines to remove from the blacklisting
};
