...
/* OTAFUZZ */
bool s1ap::send_nas_test_message(uint16_t rnti)
{
  ...
  if (user_ptr == nullptr) {
    srsran::console("Error! couldn't find user with rnti=0x%x\n", rnti);
    return false;
  }
  ...
  container->mme_ue_s1ap_id_present = true;

  container->enb_ue_s1ap_id.value = user_ptr->ctxt.enb_ue_s1ap_id;
  logger.info("Sending S1AP eNB_UE S1ap_ID : 0x%x\n", user_ptr->ctxt.enb_ue_s1ap_id);
  
  container->mme_ue_s1ap_id.value = user_ptr->ctxt.mme_ue_s1ap_id.value();
  logger.info("Sending S1AP MME_UE S1ap_ID : 0x%x\n", user_ptr->ctxt.mme_ue_s1ap_id.value());

  container->cause_present = true;
  // I will use this cause for send_nas_test_message
  container->cause.value.set(asn1::s1ap::cause_c::types_opts::radio_network);

  container->s_tmsi_present = false;
  container->crit_diagnostics_present = false;

  return sctp_send_s1ap_pdu(tx_pdu, rnti, "Error Indication");

}

bool s1ap::send_paging_message(uint16_t rnti)
{
  ...
  container->cause_present = true;
  // I will use this cause for send_paging_message
  container->cause.value.set(asn1::s1ap::cause_c::types_opts::transport);

  container->s_tmsi_present = false;
  container->crit_diagnostics_present = false;

  return sctp_send_s1ap_pdu(tx_pdu, rnti, "Error Indication for Paging Message");
...
void s1ap::notify_nas_oracle(uint16_t rnti)
{
  // Use error indication with a speicific cause to notify NAS oracle
  if (not mme_connected) {
    return;
  }
  ...
  container->cause_present = true;
  // I will use this cause for notifying that NAS ID oracle is triggered
  container->cause.value.set(asn1::s1ap::cause_c::types_opts::nas);

  container->s_tmsi_present = false;
  container->crit_diagnostics_present = false;

  sctp_send_s1ap_pdu(tx_pdu, rnti, "Error Indication for NAS ID Request");
}
...
bool s1ap::handle_initiatingmessage(const init_msg_s& msg)
{
  switch (msg.value.type().value) {
    ...
    case s1ap_elem_procs_o::init_msg_c::types_opts::error_ind:
      return handle_error_indication(msg.value.error_ind());
    default:
...
bool s1ap::handle_dlnastransport(const dl_nas_transport_s& msg)
{
  ...
  // Check if we are sending NAS oracle (ID Req). 
  bool is_id_req = false;
  if ((pdu->msg[0] == 0x07 && pdu->msg[1] == 0x55 && pdu->msg[2] == 0x01) || 
    (pdu->msg[0] == 0x27 && pdu->msg[6] == 0x07 && pdu->msg[7] == 0x55 && (pdu->msg[8] & 0x07) == 0x01)){
    srsran::console("Received ID Request\n");
    srsran::console("pdu[8] is %x\n", pdu->msg[8]);
    is_id_req = true;
  }

  // at rrc, handle the argument if target protocol is TEST_NAS and ready for test
  ...
  rrc->write_dl_info(u->ctxt.rnti, std::move(pdu), is_id_req);
...
/* OTAFUZZ */
bool s1ap::handle_error_indication(const asn1::s1ap::error_ind_s& msg)
{
  ue* u = handle_s1apmsg_ue_id(msg->enb_ue_s1ap_id.value.value, msg->mme_ue_s1ap_id.value.value);
  if (u == nullptr) {
    logger.error("Error while handling UE ID of Error Indication message");
    return false;
  }

  if (msg->cause.value.type().value == asn1::s1ap::cause_c::types_opts::radio_network){
    rrc->send_connection_release_tau(u->ctxt.rnti);
  } else if(msg->cause.value.type().value == asn1::s1ap::cause_c::types_opts::transport){
    rrc->send_connection_release_other(u->ctxt.rnti);
    send_paging_message(u->ctxt.rnti);
  } else if (msg->cause.value.type().value == asn1::s1ap::cause_c::types_opts::nas){
    rrc->set_ready_for_test(u->ctxt.rnti, false);
    // rrc->set_reattach_timeout(u->ctxt.rnti, true);
    
  } else if (msg->cause.value.type().value == asn1::s1ap::cause_c::types_opts::protocol){
      // When EPC wants eNB to send a first test message
    if (!rrc->set_ready_for_test(u->ctxt.rnti, true)){
      logger.error("Error Indication message with cause protocol and rrc->set_ready_for_test failed");
      return false;
    }
    rrc->send_next_test_msg(u->ctxt.rnti);
  } else {
    logger.error("Error Indication message with cause other than radio_network or transport");
    return false;
  }
...
s1ap::ue* s1ap::handle_s1apmsg_ue_id(uint32_t enb_id, uint32_t mme_id)
{
  ...
  // print user_ptr->ctxt.mme_ue_s1ap_id
  // srsran::console("user_ptr->ctxt.mme_ue_s1ap_id = %d\n", user_ptr->ctxt.mme_ue_s1ap_id.value());
  if (user_ptr != nullptr) {
