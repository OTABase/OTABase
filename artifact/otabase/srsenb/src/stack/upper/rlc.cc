...
void rlc::write_pdu(uint16_t rnti, uint32_t lcid, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  // Send next test message if RLC control PDU is received. 
  // check if the first bit of the payload is 0 
  if (((payload[0] >> 7) & 0x1) == 0) {
    rrc->send_next_test_msg(rnti);
    count++;
  }
}
...
void rlc::user_interface::send_next_test_msg()
{
  rrc->send_next_test_msg(rnti);
}
