  ...
  mac_ctrl(rnti, ue_cell_list, bearer_list, parent->cfg, parent->mac, *parent->cell_common_list, sched_ue_cfg)
{
  ...
  if (connect(socketfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
    parent->logger.error("Error: RRC_UE could not connect to the python server");
    fuzz = false;
  } else {
    parent->logger.info("RRC_UE connected to the python server");
    fuzz = true;
  }
...
rrc::ue::~ue() {
  parent->logger.info("UE 0x%x destroyed", rnti);
  ...
  if (parent->s1ap->user_exists(rnti)) {
    switch (type) {
      case UE_INACTIVITY_TIMEOUT:
        ...
        if (parent->recent_rnti == rnti){
          srsran::console("Sending release to RNTI%x (%x)!\n", rnti, parent->recent_rnti);
          parent->logger.info("Sending release to RNTI%x (%x)!", rnti, parent->recent_rnti);
          send_connection_release();
          srsran::console("Time to reattach a UE!\n");
          parent->logger.info("Time to reattach a UE!");
          parent->send_paging((uint64_t)901550000044693, parent->cur_tmsi);
          // parent->send_paging((uint64_t)901550000044693, 0);
          // parent->send_paging(0, parent->cur_tmsi);
          parent->set_reattach_timer();
        }
      ...
      case MSG5_RX_TIMEOUT_T301:
        ...
        break;
      // case OTAFUZZ_REATTACH_TIMEOUT:
      //   srsran::console("REATTACH_TIMER: Time to reattach a UE!\n");
      //   parent->logger.info("REATTACH_TIMER: Time to reattach a UE!");
      //   parent->send_paging((uint64_t)901550000044693, parent->cur_tmsi);
...
void rrc::ue::max_rlc_retx_reached()
{
  ...
  if (parent->cfg.target_protocol == TEST_RRC){
    if (!parent->is_backtracking){
      // We need to turn on the backtracking mode
      parent->is_backtracking = true;
      parent->backtracking_num = 0;
      parent->logger.info("Turn on the backtracking mode");
      srsran::console("Turn on the backtracking mode\n");
    } else {
      // Handle when the NO ACK oracle triggered during the backtracking
      // If the last message is causing the No ACK oracle again, mark this packet as a best candidate. 
      if (parent->backtracking_num == 1){
        parent->logger.info("Found the best candidate");
        srsran::console("Found the best candidate\n");
        parent->save_recent_messages(parent->logging_path, parent->backtracking_msg);
      } else {
        parent->logger.info("Found the second-best candidates");
        srsran::console("Found the second-best candidates\n");
        parent->save_recent_messages(parent->logging_path, parent->backtracking_msg, parent->backtracking_num);
      }

      // When doing replay_mode, we do not blacklist test cases to try all the cases. 
      if (!parent->cfg.replay_mode){
        // Also, blacklist the test cases with the same message type + target field 
        parent->blacklist_test_cases(parent->backtracking_msg);
      }

      // Temporary black list msg+field for N messages. 
      if (parent->cfg.temp_blacklist){
        parent->temp_blacklist_test_cases(parent->backtracking_msg);
      }
      // Need to change the curLineNum
    }
  } else if (parent->cfg.target_protocol == TEST_NAS){
    parent->s1ap->notify_nas_oracle(rnti);
  }

  // parent->save_recent_messages("S8_test1");
...
bool rrc::ue::set_reattach_timeout(){return true;}
...
void rrc::ue::parse_ul_dcch(uint32_t lcid, srsran::unique_byte_buffer_t pdu)
{
  ...
  // Stop reattach timer whenever receiving a message. 
  parent->set_reattach_timer();
  ...
  switch (ul_dcch_msg.msg.c1().type()) {
    case ul_dcch_msg_type_c::c1_c_::types::rrc_conn_setup_complete:
      ...
      //However, this will not stop the other timer. 
      activity_timer.stop();
      set_activity_timeout(UE_INACTIVITY_TIMEOUT);
      set_activity();
      
      parent->repaging_timer.stop();
      parent->set_reattach_timer();
      // temporary
      // allow_next_msg = true;
      parent->recent_rnti = rnti;
      break;
    case ul_dcch_msg_type_c::c1_c_::types::rrc_conn_reest_complete:
      // Also, disable backtracking mode. This is because for some messages, UE sends Reestab request insteam of ACK. 
      if (parent->cfg.target_protocol == TEST_RRC){
        if (parent->is_backtracking){
          parent->is_backtracking = false;
          parent->backtracking_num = 0;
          parent->backtracking_num_total = 0;
          parent->backtracking_msg = "";
        }
      }
      save_ul_message(std::move(original_pdu));
    ...
    case ul_dcch_msg_type_c::c1_c_::types::ul_info_transfer:
      ...
      // After starting the test, check if pdu is ID Response
      // When we deliver ID Response to EPC, there were significant delays for sending the next packet.
      // Thus, we process at eNB

      srsran::console("Here, ready for test is %s\n", ready_for_test?"true":"false");
      srsran::console("Here, waiting_for_nas_oracle is %s\n", waiting_for_nas_oracle?"true":"false");

      if (ready_for_test && parent->cfg.target_protocol == TEST_NAS){
        // When UE sends NAS message during the test
        // Because while testing, handling NAS message brings a lot of delay (~200ms). 

        parent->logger.info("Received UL Information Transfer (NAS) message. However, not handling it.\n");
        srsran::console("Received UL Information Transfer with (NAS) message. However, not handling it.\n");
        
      } else if (waiting_for_nas_oracle && parent->cfg.target_protocol == TEST_NAS){
        //check the message type is 56 (can be 0756 or 2756)
        if((pdu->msg[0] == 0x07 && pdu->msg[1] == 0x56) || 
          ((pdu->msg[0] == 0x27 || pdu->msg[0] == 0x17) && pdu->msg[6] == 0x07 && pdu->msg[7] == 0x56 && (pdu->msg[9] & 0x07) == 0x01)){
          srsran::console("pdu->msg[9] is %x\n", pdu->msg[9]);

          parent->logger.info("Received ID Response");
          srsran::console("Received ID Response");

          // Original 
          
          parent->nas_oracle_timer.stop();
          
        //  // Test part for functional testing
        //   num_sent_msg++;
        //   if (num_sent_msg <= 5){
        //     parent->nas_oracle_timer.stop();
        //   } else {
        //     parent->repaging_timer.stop();
        //     srsran::console("Not handling NAS Oracle Response on purpose!\n")
        //     break;
        //   }
         // New end
    
          waiting_for_nas_oracle = false;
          ready_for_test = true;
          // Let NAS layer to send the next message
          parent->send_next_test_msg(rnti);
        } else {
          parent->logger.info("Received UL Information Transfer but not handling\n");
          srsran::console("Received UL Information Transfer but not handling\n");
          // parent->s1ap->write_pdu(rnti, std::move(pdu));
        }
      } else {
        parent->s1ap->write_pdu(rnti, std::move(pdu));
      ...
      // When UE doesn't send a response, we want to page & toggle airplane mode. 
      parent->set_reattach_timer();

      break;
    case ul_dcch_msg_type_c::c1_c_::types::rrc_conn_recfg_complete:
      save_ul_message(std::move(original_pdu));
      //TODO: When testing, does not handle the complete message. 
      if (!ready_for_test && is_doing_state_transition){
        parent->logger.info("Received");
      ...
      parent->paging_attempt = 0;
      parent-> airplanemode_attempt = 0;
      set_activity_timeout(UE_INACTIVITY_TIMEOUT);
      break;
    ...
    case ul_dcch_msg_type_c::c1_c_::types::ue_cap_info:
      // Keep original code (handling UE cap) normally. 
      if (!waiting_for_rrc_oracle){
        ...
        // If we've sent RRC oracle (UE Cap Enquiry) and was waiting for the response, here
        parent->logger.info("Received UE Cap Info. However, not handling it.");
        srsran::console("Received UE Cap Info (oracle check) \n");
        
        if (parent->rrc_oracle_timer.is_running()){
          parent->rrc_oracle_timer.stop();
        }
        waiting_for_rrc_oracle = false;
        ready_for_test = true;

        // Let RRC layer to send the next message
        parent->send_next_test_msg(rnti);
      }
      break;
...
void rrc::ue::handle_rrc_con_reest_complete(rrc_conn_reest_complete_s* msg, srsran::unique_byte_buffer_t pdu)
{
  ...
  parent->logger.info("RRCConnectionReestablishmentComplete for rnti=0x%x", rnti);
  // When Reestablishment procedure is completed, RNTI is updated. 
  parent->logger.info("Recent rnti and current rnti is 0x%x, %x", parent->recent_rnti, rnti);
  parent->recent_rnti = rnti;

  ready_for_test = true;
  is_reestab_procedure = true;
                                     ...
                                     srsran::const_byte_span      nas_pdu)
{
  ...
  if (recfg_r8.ded_info_nas_list_present){
    // Retreive the GUTI from the NAS PDU (NAS Attach Accept message)
    // Check if it is IEI of EPS mobile identity (GUTI)
    if (recfg_r8.ded_info_nas_list[0].data()[48] == 0x50 && recfg_r8.ded_info_nas_list[0].data()[49] == 0x0b){
      srsran::console("GUTI CHK\n");
      parent->prev_tmsi = parent->cur_tmsi;
      parent->cur_tmsi = 0;
      for (int i = 0; i < 4; i++){
        parent->cur_tmsi = (parent->cur_tmsi << 8) | recfg_r8.ded_info_nas_list[0].data()[57+i];
      }

      srsran::console("prev_tmsi is %x\n", parent->prev_tmsi);
      srsran::console("cur_tmsi is %x\n", parent->cur_tmsi);
    } else {
      srsran::console("GUTI NOT CHK\n");
    }
  }
...
void rrc::ue::handle_rrc_reconf_complete(rrc_conn_recfg_complete_s* msg, srsran::unique_byte_buffer_t pdu)
{
  ...
  //send connection release with load balancing tau required cause. 
  // send_connection_release();
...
void rrc::ue::send_security_mode_command()
{
  ...
  is_doing_state_transition = true;
...
// eNB request EPC to send next test message through s1ap connection. 
void rrc::ue::send_nas_test_message()
{
  parent->s1ap->send_nas_test_message(rnti);
  set_activity_timeout(UE_INACTIVITY_TIMEOUT);
  parent->logger.info("Sent a message for Requesting NAS test message through S1AP\n");
  srsran::console("Send the next nsg msg!\n");
}

void rrc::ue::send_rrc_test_message()
{
  parent->logger.info("Sending RRC test message\n");
  std::string payload;

  parent->total_sent_msg++;
  num_sent_msg++;

  // Used for checking when to send a oracle message. 
  num_msg_N_oracle++;

  // When doing replay, do active liveness check for every test message
  if (parent->cfg.replay_mode){
    parent->check_period = 2;
  }

  // Occasionally, send a RRC UECapablity Enquiry message for liveness check
  if (num_msg_N_oracle % parent->check_period == 0) {
    send_ue_cap_enquiry({asn1::rrc::rat_type_opts::options::eutra});
    waiting_for_rrc_oracle = true;
    ready_for_test = false;

    // After semdomg the IE cap, run the timer. 
    // When it's expired, it means that we need to backtrack. 
    parent->set_rrc_oracle_timer();

    parent->logger.info("* Sending RRC liveness check: UE Capability Enquiry");
    srsran::console("* Sending RRC liveness check: UE Capability Enquiry\n");
    
  } else {
    // Otherwise, send a test message
    parent->get_test_msg_from_file(payload);

    // Log the payload
    parent->logger.info("Sending payload: %s\n", payload.c_str());
    parent->logger.info("Payload length: %zu (B), This session: %d, Total: %d\n", (payload.length()/2), num_sent_msg, parent->total_sent_msg);
...
void rrc::ue::notify_rrc_oracle(){
  // When UE does not respond to UE Capability Enquiry, turn on the backtracking mode. 

  parent->rrc_oracle_cnt++;
  uint8_t max_oracle_trial = 2;

  if (parent->rrc_oracle_cnt > max_oracle_trial){
    if (!parent->is_backtracking){
      // We need to turn on the backtracking mode
      parent->is_backtracking = true;
      parent->backtracking_num = 0;
...
void rrc::ue::send_rrc_test_message_backtracking(){
  parent->logger.info("Backtracking RRC test message\n");
  std::string payload_msgName_path, payload;

  // Get the backtracking queue
  std::vector<std::string> backtracking_queue = parent->get_recent_messages();

  // Increase the backtracking number
  parent->backtracking_num++;
  parent->backtracking_num_total++;

  // Check if the backtracking is done
  if (parent->backtracking_num > backtracking_queue.size()){
    parent->logger.info("Backtracking done\n");
    // For now, let's not save when backtracking is done without getting a candidate
    // parent->save_recent_messages(parent->logging_path);
    parent->is_backtracking = false;
    parent->backtracking_num = 0;
    parent->backtracking_num_total = 0;
    parent->backtracking_msg = "";
    // Clear the test_message_queue as well
    parent->test_message_queue = {};

    // Send the next test message
    send_rrc_test_message();

    return;
  }

  // For each backtracking message, check the oracle
  if (parent->backtracking_num_total % 2 == 0){
    send_ue_cap_enquiry({asn1::rrc::rat_type_opts::options::eutra});
    waiting_for_rrc_oracle = true;
    ready_for_test = false;

    parent->backtracking_num--;
    srsran::console("[Backtracking Oracle #%d]", parent->backtracking_num_total/2);
  ...
  // Get the payload_msgName_path from the backtracking queue
  payload_msgName_path = backtracking_queue[backtracking_queue.size() - parent->backtracking_num];
  parent->backtracking_msg = payload_msgName_path;
  ...
  if (!std::getline(inputStream, payload, ',')) {
    parent->logger.error("Error: could not get payload from payload_msgName_path\n");
    return;
  }

  // Log the payload
  parent->logger.info("[Backtracking #%d] Sending payload: %s\n", parent->backtracking_num, payload.c_str());
  parent->logger.info("[Backtracking #%d] Payload length: %zu (B), This session: %d, Total: %d\n", parent->backtracking_num, (payload.length()/2), num_sent_msg, parent->total_sent_msg);

  if (payload.length()/2 < 40){
    srsran::console("[Backtracking #%d] Payload bytes: %s\n", parent->backtracking_num, payload.c_str());
  } else {
    srsran::console("[Backtracking #%d] Payload bytes: %s ... %s\n", parent->backtracking_num, payload.substr(0, 20).c_str(), payload.substr(payload.length()-20, payload.length()).c_str()); 
  }
...
// This function gets length as input and outputs length number of random hex characters in string format
std::string rrc::ue::rand_hex_stream(int length)
{
  std::string stream = "";
  for (int i = 0; i < length; i++) {
    stream += "0123456789ABCDEF"[rand() % 16];
  }
  return stream;
}

// eNB request EPC to send S-TMSI paging message through s1ap connection. 
void rrc::ue::send_paging_message()
{
  parent->s1ap->send_paging_message(rnti);
  parent->logger.info("Sent a message for Requesting S-TMSI paging message through S1AP\n");
}

void rrc::ue::notify_ack_timeout()
{
  if (is_using_python){
    // This is for python version 
    pthread_mutex_lock(&socket_mutex);
    parent->logger.info("ACK timeout for rnti=0x%x, msg #%d, total msg #%d", rnti, num_sent_msg, parent->total_sent_msg);

    auto current_time = std::chrono::system_clock::now();

    // Calculate the duration since the Unix epoch
    auto since_epoch = current_time.time_since_epoch();

    // Extract the seconds part (like before)
    std::time_t unix_timestamp_seconds = std::chrono::system_clock::to_time_t(current_time);

    // Extract the milliseconds part
    auto milliseconds =
        std::chrono::duration_cast<std::chrono::milliseconds>(since_epoch).count() % 1000;

    parent->logger.info("TIMEOUT triggered at %lld%03lld\n", (long long)unix_timestamp_seconds, (long long)milliseconds);

    // Index should be encoded on 16 bits and not simply converted to string
    const char* prefix = "ATO";
    ...
    if (fuzz) {
      ...
      char buffer;
      int result = recv(socketfd, &buffer, 1, MSG_PEEK | MSG_DONTWAIT);
      if (result == 0) {
          // The other side has closed the connection
          srsran::console("Connection closed by peer.\n");
          close(socketfd); // consider closing the socket on your side too
          return; // or handle the situation as needed
      }
      parent->logger.error("Socket valid");
      if (send(socketfd, message,sizeof(message), MSG_DONTWAIT) < 0) {
        srsran::console("Error: could not send message\n");
      } else{
        parent->logger.info("Sent ACK Timeout");
      }
    } else {
      parent->logger.error("Socket not valid");
    } 
...
void rrc::ue::send_connection_release_tau()
{
  ...
  rrc_release.rrc_transaction_id     = (uint8_t)((transaction_id++) % 4);
  rrc_conn_release_r8_ies_s& rel_ies = rrc_release.crit_exts.set_c1().set_rrc_conn_release_r8();
  rel_ies.release_cause              = release_cause_e::load_balancing_ta_urequired;//asn1::rrc::release_cause_e::load_balancing_ta_urequired;
  if (is_csfb) {
    if (parent->sib7.carrier_freqs_info_list.size() > 0) {
      rel_ies.redirected_carrier_info_present = true;
      rel_ies.redirected_carrier_info.set_geran();
...
bool rrc::ue::send_dl_dcch(const dl_dcch_msg_s* dl_dcch_msg, srsran::unique_byte_buffer_t pdu, std::string* octet_str)
{
  ...
  // Run reattach timer whenever sending a message
  parent->set_reattach_timer();
...
bool rrc::ue::send_dl_dcch_bytes(std::string payload, srsran::unique_byte_buffer_t pdu)
{
  if (pdu == nullptr) {
    pdu = srsran::make_byte_buffer();
    if (pdu == nullptr) {
  ...
  char buf[32] = {};
  // parent->log_rrc_message(Tx, rnti, srb_to_lcid(rb), *pdu, *dl_dcch_msg, dl_dcch_msg->msg.c1().type().to_string());
  sprintf(buf, "SRB%d - rnti=0x%x", srb_to_lcid(rb), rnti);
