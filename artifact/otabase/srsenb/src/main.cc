...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  string enb_id;
  string cfr_mode;
  string otafuzz_target_protocol;
  string ota_conn_eval;
  ...
  common.add_options()
    ...
    ("target_protocol",        bpo::value<string>(&otafuzz_target_protocol),                            "Testing RRC or NAS?")
    ("o",                      bpo::value<string>(&output_directory_name),                              "The name of the output directory")
    ("r",                      bpo::value<bool>(&args->general.replay_mode)->default_value(false),      "Turn on replay mode for validating bug candidates")
    ("temp_blacklist",         bpo::value<bool>(&args->general.temp_blacklist)->default_value(true),    "Enable/disable temporary blacklisting during normal test")
    ("test_device",            bpo::value<string>(&test_device),                                        "The name of the test device")
    ("ota_conn_eval",          bpo::value<string>(&ota_conn_eval)->default_value("-"),                  "a: idle_time, b: processing_speed ")
  ...
  {
    if (otafuzz_target_protocol == "RRC" || otafuzz_target_protocol == "rrc") {
      args->general.target_protocol = 0;
    } else if (otafuzz_target_protocol == "NAS" || otafuzz_target_protocol == "nas") {
      args->general.target_protocol = 1;
    } else if (otafuzz_target_protocol == "") {
      fprintf(stderr, "Error getting target protocol. Please put --target_protocol option. \n");
      exit(1);
    } else {
      fprintf(stderr, "Error getting target protocol: unsupported protocol\n");
      exit(1);
    }
  }

  {
    if (ota_conn_eval == "a" || ota_conn_eval == "idle_time") {
      args->general.ota_conn_eval = 1;
    } else if (ota_conn_eval == "b" || ota_conn_eval == "processing_speed") {
      args->general.ota_conn_eval = 2;
    } else if (ota_conn_eval == "-") {
      args->general.ota_conn_eval = 0;
    } else {
      fprintf(stderr, "Error: unsupported evaluation option\n");
      exit(1);
    }
  }

  {
    if (output_directory_name == "") {
      fprintf(stderr, "Error getting output directory name. Please put --o option. \n");
      exit(1);
    } 
    args->general.output_directory_name = output_directory_name;
  }

  {
    if (test_device == "") {
      fprintf(stderr, "Error getting the name of the test device. Please put --test_device option. \n");
      exit(1);
    } 
    args->general.test_device = test_device;
  }
