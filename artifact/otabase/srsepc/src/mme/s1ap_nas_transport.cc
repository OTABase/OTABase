...
void s1ap_nas_transport::init()
{
  ...
  m_nas_init.reattach_enabled = m_s1ap->m_s1ap_args.reattach_enabled;
  m_nas_init.test_state     = m_s1ap->m_s1ap_args.test_state;
  m_nas_init.ota_timer      = m_s1ap->m_s1ap_args.ota_timer;
  m_nas_init.replay_mode    = m_s1ap->m_s1ap_args.replay_mode;
  m_nas_init.blacklist_mode = m_s1ap->m_s1ap_args.blacklist_mode;
  m_nas_init.reattach_measure = m_s1ap->m_s1ap_args.reattach_measure;
                                                     ...
                                                     struct sctp_sndrcvinfo*               enb_sri)
{
  ...
  switch (msg_type) {
    ...
    case LIBLTE_MME_MSG_TYPE_DETACH_ACCEPT:
      m_logger.info("UL NAS: Detach Accept");
      srsran::console("UL NAS: Detach Accept\n");
      nas_ctx->handle_detach_accept(nas_msg.get());
      break;
    case LIBLTE_MME_MSG_TYPE_TRACKING_AREA_UPDATE_COMPLETE:
      m_logger.info("UL NAS: Tracking Area Update Complete");
      srsran::console("UL NAS: Tracking Area Update Complete\n");
