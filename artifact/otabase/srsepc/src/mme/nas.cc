...
nas::nas(const nas_init_t& args, const nas_if_t& itf) :
  ...
  m_lac(args.lac),
  m_reattach_enabled(args.reattach_enabled),
  m_test_state(args.test_state),
  m_tdo_ota(args.ota_timer),
  m_replay_mode(args.replay_mode),
  m_blacklist_mode(args.blacklist_mode),
  m_reattach_measure(args.reattach_measure)
{
                                                ...
                                                const nas_if_t&                                       itf)
{
  ...
  /* OTAFUZZ */
  // Check if we target pre-aka state or not. 

  if (nas_ctx->m_test_state == PRE_AKA_STATE) {
    if (nas_ctx->m_reattach_enabled) {
      if (nas_ctx->m_mme->is_nas_timer_running(T_DoOTA, nas_ctx->m_emm_ctx.imsi)){
          nas_ctx->m_mme->remove_nas_timer(T_DoOTA, nas_ctx->m_emm_ctx.imsi);
      }
      if (!nas_ctx->start_timer(T_DoOTA)){
        nas_logger.error("Couldn't start OTA timer in %s().", __FUNCTION__);
        srsran::console("Couldn't start OTA timer in %s().", __FUNCTION__);
        return false;
      } 
    } 
    //Request eNB to start sending a test message
    nas_ctx->m_s1ap->send_error_indication(nas_ctx, 3);
    srsran::console("Request eNB to start sending a test message @ pre-aka\n");
  } else if (nas_ctx->m_test_state >= POST_AKA_STATE) { // post aka, tau, sr and detach
...

{
  ...
  // TEMP for testing imsi attach
  // nas_ctx->pack_identity_request(nas_tx.get());
  nas_ctx->pack_attach_reject(nas_tx.get());
  nas_ctx->is_guti_unknown_attach = true;
  s1ap->send_downlink_nas_transport(
      nas_ctx->m_ecm_ctx.enb_ue_s1ap_id, nas_ctx->m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), nas_ctx->m_ecm_ctx.enb_sri);
                                              ...
                                              const nas_if_t&                                       itf)
{
  ...
  if (msg_valid == true) {
    ...
    if (nas_ctx->m_test_state == SR_STATE){
      nas_ctx->allow_detach_during_sr_procedure = false;
    }
    // nas_ctx->is_guti_known_attach = true;
    ...
    if (ecm_ctx->eit) {
      ...
      if (nas_ctx->m_test_state == PRE_AKA_STATE){
        if (nas_ctx->m_reattach_enabled) {
          if (nas_ctx->m_mme->is_nas_timer_running(T_DoOTA, emm_ctx->imsi)){
            nas_ctx->m_mme->remove_nas_timer(T_DoOTA, emm_ctx->imsi);
          }
          if (!nas_ctx->start_timer(T_DoOTA)){
            nas_logger.error("Couldn't start OTA timer in %s().", __FUNCTION__);
            srsran::console("Couldn't start OTA timer in %s().", __FUNCTION__);
        ...
        nas_ctx->m_s1ap->send_error_indication(nas_ctx, 3);
        srsran::console("Request eNB to start sending a test message @ PRE AKA\n");
      } else if (nas_ctx->m_test_state >= POST_AKA_STATE){ // post aka, tau, sr 
        if (nas_ctx->handle_uplink_detach_request){
          nas_ctx->handle_uplink_detach_request = false;
          nas_ctx->pack_attach_reject(nas_tx.get());
        } else {
        ...
        srsran::console("ERROR! Wrong state for test message");
        nas_logger.error("ERROR! Wrong state for test message");
      }
                                 ...
                                 const nas_if_t&         itf)
{
  ...
  if (imsi == 0) {
    // Currently, this might not be a case. 
  ...
  if (nas_ctx == NULL || nas_ctx->m_emm_ctx.state != EMM_STATE_REGISTERED) {
    // Also, this might not be a case currently. 
  ...
  if (mac_valid) {
    ...
    nas_ctx->allow_detach_during_sr_procedure = true;
    // Save UE ctx to MME UE S1AP id
    s1ap->add_nas_ctx_to_mme_ue_s1ap_id_map(nas_ctx);

    // This handles the service request. Moving this to the timer logic. 
    /*
    ...
    */
    if (nas_ctx->m_test_state == SR_STATE){
      if (nas_ctx->m_reattach_enabled){
        if (mme->is_nas_timer_running(T_DoOTA, emm_ctx->imsi)){
          mme->remove_nas_timer(T_DoOTA, emm_ctx->imsi);
        }
        if (!nas_ctx->start_timer(T_DoOTA)){
          nas_logger.error("Could not start T_DoOTA timer");
          srsran::console("Could not start T_DoOTA timer\n");
          return false;
        }
      }
      // Request eNB to start sending a test message
      nas_ctx->m_s1ap->send_error_indication(nas_ctx, 3);
      srsran::console("Request eNB to start sending a test message @ SR\n");
    } else{
      // In other state, complete the SR procedure and send detach and reattach. 
      nas_ctx->m_s1ap->send_initial_context_setup_request(nas_ctx->m_emm_ctx.imsi, 5);
    }
  } else {
                                              ...
                                              const nas_if_t&         itf)
{
  ...
  // Get NAS TAU Request message
  LIBLTE_ERROR_ENUM err = liblte_mme_unpack_tracking_area_update_request_msg((LIBLTE_BYTE_MSG_STRUCT*)nas_rx, &tau_req);
  if (err != LIBLTE_SUCCESS) {
    nas_logger.error("Error unpacking Tracking Area Update Request. Error: %s", liblte_error_text[err]);
    return false;
  }

  m_tmsi_real = tau_req.old_guti.guti.m_tmsi;
  imsi = s1ap->find_imsi_from_m_tmsi(m_tmsi_real);

  nas_logger.info("Tracking Area Update Request -- S-TMSI 0x%x", m_tmsi_real);
  ...
  // print the imsi to the console
  srsran::console("Tracking Area Update Request -- IMSI: %015" PRIu64 "\n", imsi);
  if (nas_ctx == NULL){
    // Unknown UE
    srsran::console("Tracking Area Update Request -- Unknown UE\n");
    ...
    nas_tmp.pack_tracking_area_update_reject(nas_tx.get(), LIBLTE_MME_EMM_CAUSE_ILLEGAL_UE);
    s1ap->send_downlink_nas_transport(enb_ue_s1ap_id, nas_tmp.m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), *enb_sri);
    

  } else {
    // Usually, here. 
    if (nas_ctx->m_ecm_ctx.state == ECM_STATE_CONNECTED) {
      // Release previous context
      nas_logger.info("TAU Request -- Releasing previouse ECM context. eNB S1AP Id %d, MME UE S1AP Id %d",
    ...
    // Known UE
    srsran::console("Tracking Area Update Request -- Known UE\n");
    srsran::unique_byte_buffer_t nas_tx = srsran::make_byte_buffer();
    nas_ctx->m_ecm_ctx.mme_ue_s1ap_id = s1ap->get_next_mme_ue_s1ap_id();
    nas_ctx->m_ecm_ctx.enb_ue_s1ap_id = enb_ue_s1ap_id;
    ...
    //HERE 
    nas_ctx->eps_update_type = tau_req.eps_update_type.type;

    // Before timeout (T3430), complete the TAU procedure and send detach.  
    // Tracking area updating attempt counter
    // Service request attempt counter: 5, after that, T3325 (default 60s) starts. 
    // Attach attempt counter
    // Put the logic here. 
    /*
    nas_ctx->eps_update_type = tau_req.eps_update_type.type;
    nas_ctx->pack_tracking_area_update_accept(nas_tx.get(), tau_req.eps_update_type.type);
    s1ap->send_downlink_nas_transport(enb_ue_s1ap_id, nas_ctx->m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), *enb_sri);
    */
    if (nas_ctx->m_test_state == TAU_STATE){
      if (nas_ctx->m_reattach_enabled){
        if (nas_ctx->m_mme->is_nas_timer_running(T_DoOTA, nas_ctx->m_emm_ctx.imsi)){
          nas_ctx->m_mme->remove_nas_timer(T_DoOTA, nas_ctx->m_emm_ctx.imsi);
        }
      ...
      // Request eNB to start sending a test message
      nas_ctx->m_s1ap->send_error_indication(nas_ctx, 3);
      srsran::console("Request eNB to start sending a test message @ TAU\n");
    } else{
      srsran::console("When UE is not in TAU state, send TAU accept and make it reattach.\n");
      nas_logger.error("When UE is not in TAU state, send TAU accept and make it reattach.");
...
bool nas::handle_attach_complete(srsran::byte_buffer_t* nas_rx)
{
  ...
  // In PRE_AKA_STATE, this state in not reached if reattach_enabled is false. 
  if (m_test_state == PRE_AKA_STATE && m_reattach_enabled){
    ...
    srsran::console("Sending Detach Request\n");
    m_logger.info("Sending Detach Request");
  } else if (m_test_state == POST_AKA_STATE){
    if (m_reattach_enabled) {
      if (!start_timer(T_DoOTA)){
        m_logger.error("Couldn't start OTA timer in %s().", __FUNCTION__);
        srsran::console("Couldn't start OTA timer in %s().", __FUNCTION__);
        return false;
      } 
    } 
    //TODO: Request eNB to start sending a test message
    // nas_ctx->m_s1ap->send_error_indication(nas_ctx);
    m_s1ap->send_error_indication(this, 3);
    srsran::console("Request eNB to start sending a test message @ POST AKA\n");
  } else if (m_test_state == TAU_STATE){
    srsran::console("Sending error indication!");
    m_s1ap->send_error_indication(this, 0);
    
  } else if (m_test_state == SR_STATE){
    // First, we need to release the rrc connection. Then, send the paging. 
    // m_s1ap->send_paging_message(this);
    m_s1ap->send_error_indication(this, 1);
  }
...
{
  m_logger.info("Received Identity Response\n");
  srsran::console("Received Identity Response\n");
  /*
  srsran::unique_byte_buffer_t      nas_tx;
...
bool nas::handle_detach_accept(srsran::byte_buffer_t* nas_rx){
  m_logger.info("Received Detach Accept");
...
bool nas::handle_detach_request(srsran::byte_buffer_t* nas_msg)
{
  ...
  if (detach_req.detach_type.switch_off == 0){
    ...
    LIBLTE_MME_DETACH_ACCEPT_MSG_STRUCT detach_accept = {};
    err = liblte_mme_pack_detach_accept_msg(&detach_accept, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_tx.get());

    if (err != LIBLTE_SUCCESS) {
      m_logger.error("Error packing Detach Accept");
    ...
    handle_uplink_detach_request = true;
    //log 
    m_logger.info("Sending Detach Accept");
    srsran::console("Sending Detach Accept\n");
    m_s1ap->send_downlink_nas_transport(
        m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);
  } else {
    m_logger.info("UE is switched off");
  }
...
bool nas::handle_tracking_area_update_complete(srsran::byte_buffer_t* nas_rx)
{
  ...
  if (m_test_state == TAU_STATE){
    // After completing the TAU procedure, send RRC_Connection_Release with load balancing TAU request cause. 
    // This will trigger the UE to perform a TAU procedure.
    m_s1ap->send_error_indication(this, 0);
  } else {
    pack_detach_request(nas_tx.get());
    m_s1ap->send_downlink_nas_transport(
...
bool nas::pack_detach_request(srsran::byte_buffer_t* nas_buffer)
{
  ...
  detach_req.emm_cause_present          = false;;
  // detach_req.emm_cause                  = LIBLTE_MME_EMM_CAUSE_IMSI_UNKNOWN_IN_HSS;

  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED;
  m_sec_ctx.dl_nas_count++;

  LIBLTE_ERROR_ENUM err = liblte_mme_pack_detach_request_net_msg(&detach_req, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
  if (err != LIBLTE_SUCCESS) {
  ...
  m_logger.info("Packed Detach Request (UE terminated)");

  return true;
}

bool nas::pack_detach_accept(srsran::byte_buffer_t* nas_buffer)
{
  m_logger.info("Packing Detach Request");
...
bool nas::pack_service_accept(srsran::byte_buffer_t* nas_buffer)
{
  m_logger.info("Packing Service Accept");

  LIBLTE_MME_SERVICE_ACCEPT_MSG_STRUCT service_accept;
  service_accept.eps_bearer_context_status_present = false;
  service_accept.t3448_present = false;

  m_sec_ctx.dl_nas_count++;
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_service_accept_msg(
      &service_accept, LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED, 
          m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);

  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing Service Accept");
...
// This packs NAS test message and put message authentication code
bool nas::pack_test_ota_message(srsran::byte_buffer_t* nas_buffer, std::string payload, uint8 sec_hdr)
{
  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
  if (sec_hdr != LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS && sec_hdr <= 0xf){
    sec_hdr_type = sec_hdr;
    m_sec_ctx.dl_nas_count++;
  } else{
    sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
  }
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_ota_fuzz_emm_msg(
      payload, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing OTA test message \n");
    srsran::console("Error packing OTA test message \n");
    return false;
  }

  // m_nas_log->console("NAS payload is %s \n", payload.c_str());
  ...
  m_logger.info("Packed OTA test message \n");
  return true;
}
...
bool nas::pack_tracking_area_update_accept(srsran::byte_buffer_t* nas_buffer, LIBLTE_MME_EPS_UPDATE_TYPE_ENUM eps_update_type)
{
  LIBLTE_MME_TRACKING_AREA_UPDATE_ACCEPT_MSG_STRUCT tau_accept;

  
  tau_accept.eps_update_result = eps_update_type;
  ...
  tau_accept.guti.guti.mcc = m_mcc;
  tau_accept.guti.guti.mnc = m_mnc;
  ...
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_tracking_area_update_accept_msg(
      &tau_accept, LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing Tracking Area Update Accept");
    srsran::console("Error packing Tracking Area Update Accept\n");
    return false;
  }
...
bool nas::start_timer(enum nas_timer_type type)
{
  ...
  switch (type) {
    ...
    case T_DoOTA:
      err = start_tdo_ota();
      break;
    case T_NAS_ORACLE_TIMEOUT:
      err = start_tnas_oracle_timeout();
      break;
    case T_RECONN_TIMEOUT:
      err = start_treconn_timeout();
      break;
    default:
      m_logger.error("Invalid timer type at start_timer");
  }
  return err;
}
...
bool nas::expire_timer(enum nas_timer_type type)
{
  ...
  switch (type) {
    ...
    case T_DoOTA:
      ...
      break;
    case T_NAS_ORACLE_TIMEOUT:
      err = expire_tnas_oracle_timeout();
      break;
    case T_RECONN_TIMEOUT:
      err = expire_treconn_timeout();
      break;
    default:
...
// TDoOTA -> OTA testing timer
bool nas::start_tdo_ota()
{
  m_logger.info("Starting TDoOTA Timer: Timeout value %d", m_tdo_ota);

   // Reset the msg number for this session
  msg_cnt = 0;

  int fdt = timerfd_create(CLOCK_MONOTONIC, 0);
  if (fdt < 0) {
    m_logger.error("Error creating timer. %s\n", strerror(errno));
    return false;
  }
  struct itimerspec t_value;
  t_value.it_value.tv_sec     = m_tdo_ota;
  t_value.it_value.tv_nsec    = 0;
  t_value.it_interval.tv_sec  = 0;
  t_value.it_interval.tv_nsec = 0;
  ...
  m_mme->add_nas_timer(fdt, T_DoOTA, m_emm_ctx.imsi); 
  return true;
}

bool nas::start_tnas_oracle_timeout()
{
  m_logger.info("Starting TNAS_oracle_timeout Timer: Timeout value (ns) %d\n", m_tnas_oracle_timeout);
  ...
  struct itimerspec t_value;
  // Now, timeout is 1 sec for ID Req oracle. 
  t_value.it_value.tv_sec     = m_tnas_oracle_timeout;
  t_value.it_value.tv_nsec    = 0;
  t_value.it_interval.tv_sec  = 0;
  t_value.it_interval.tv_nsec = 0;
  ...
  m_mme->add_nas_timer(fdt, T_NAS_ORACLE_TIMEOUT, m_emm_ctx.imsi); // TODO timers without IMSI?
  return true;
}

// TRECONN_TIMEOUT : When UE doesn't respond, 
bool nas::start_treconn_timeout()
{
  m_logger.info("Starting T_reconn Timer: Timeout value %d", (m_treconn_timeout + 1));
  ...
  struct itimerspec t_value;
  t_value.it_value.tv_sec     = (m_treconn_timeout + 1);
  t_value.it_value.tv_nsec    = 0;
  t_value.it_interval.tv_sec  = 0;
  t_value.it_interval.tv_nsec = 0;
  ...
  m_mme->add_nas_timer(fdt, T_RECONN_TIMEOUT, m_emm_ctx.imsi); 
  return true;
}
...
bool nas::expire_t3413()
{
  ...
  // When expired, send another paging. 
  // When timer expired, stop the timer
  if (m_mme->is_nas_timer_running(T_3413, m_emm_ctx.imsi)) {
      m_mme->remove_nas_timer(T_3413, m_emm_ctx.imsi);
  }
  m_s1ap->send_paging_message(this);
  return true;
}

bool nas::expire_tdo_ota()
{
  m_logger.info("TDoOTA expired. \n");
  srsran::console("TDoOTA expired .\n");

  // USE this message to stop sending the next message
  m_s1ap->send_error_indication(this, 2);

  if (m_reattach_enabled && m_test_state == POST_AKA_STATE) {
  ...
  } else if (m_reattach_enabled && m_test_state == PRE_AKA_STATE){

    if (is_guti_known_attach){
      if (m_ecm_ctx.eit){
        srsran::unique_byte_buffer_t nas_tx = srsran::make_byte_buffer();
        pack_esm_information_request(nas_tx.get());
  ...
  } else if (m_reattach_enabled && m_test_state == TAU_STATE){
    // Before timeout (T3430), complete the TAU procedure and send detach.  
    // Tracking area updating attempt counter
    // Service request attempt counter: 5, after that, T3325 (default 60s) starts. 
    // Attach attempt counter
    srsran::unique_byte_buffer_t nas_tx = srsran::make_byte_buffer();
    pack_tracking_area_update_accept(nas_tx.get(), eps_update_type);
    m_s1ap->send_downlink_nas_transport(m_ecm_ctx.enb_ue_s1ap_id, m_ecm_ctx.mme_ue_s1ap_id, nas_tx.get(), m_ecm_ctx.enb_sri);

  } else if (m_reattach_enabled && m_test_state == SR_STATE){
    // Complete the SR procedure. 
    m_s1ap->send_initial_context_setup_request(m_emm_ctx.imsi, 5);
    m_sec_ctx.ul_nas_count++;
  }

  //TODO: When UE doesn't attach, send a paging. 
  // if (m_reattach_enabled) {
  //   if (!start_timer(T_RECONN_TIMEOUT)){
  //     m_logger.error("Couldn't start Reconn timer in %s().", __FUNCTION__);
  //     srsran::console("Couldn't start Reconn timer in %s().", __FUNCTION__);
  //     return false;
  //   } 
  // }
  // if (m_mme->is_nas_timer_running(T_DoOTA, m_emm_ctx.imsi)) {
  //     m_mme->remove_nas_timer(T_DoOTA, m_emm_ctx.imsi);
  // }
...
bool nas::expire_tnas_oracle_timeout()
{
  m_logger.info("T_NAS_ORACLE_TIMEOUT expired.\n");
  srsran::console("T_NAS_ORACLE_TIMEOUT expired.\n");


  // // When ID REQ oracle expired, stop the timer
  // if (m_mme->is_nas_timer_running(T_NAS_ORACLE_TIMEOUT, m_emm_ctx.imsi)) {
  //     m_mme->remove_nas_timer(T_NAS_ORACLE_TIMEOUT, m_emm_ctx.imsi);
  // }

  // // Logics for backtracking & logging

  return true;
}

bool nas::expire_treconn_timeout()
{
  m_logger.info("T_reconn expired.");
  srsran::console("T_reconn expired.");

  // When ID REQ oracle expired, stop the timer
  if (m_mme->is_nas_timer_running(T_RECONN_TIMEOUT, m_emm_ctx.imsi)) {
      m_mme->remove_nas_timer(T_RECONN_TIMEOUT, m_emm_ctx.imsi);
  }
