...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  string   log_filename;
  string   lac;
  uint16_t ota_timer = 0;
  string   target_test_state;
  ...
  common.add_options()
    ("enable_reattach",     bpo::value<bool>(&args->mme_args.s1ap_args.reattach_enabled)->default_value(true), "enable reattach")
    ("r",                   bpo::value<bool>(&args->mme_args.s1ap_args.replay_mode)->default_value(false), "enable replay mode")
    ("reattach_measure",    bpo::value<bool>(&args->mme_args.s1ap_args.reattach_measure)->default_value(false), "Used for evaluating reattach feature")
    ("bl",                  bpo::value<bool>(&args->mme_args.s1ap_args.blacklist_mode)->default_value(false), "enable hard blacklist mode")
    ("test_state",          bpo::value<string>(&target_test_state),                        "Testing Pre-AKA (or 0), Post-AKA (or 1), TAU (or 2), SR (or 3), or Detach (or 4)? ")
    ("mme.ota_timer",       bpo::value<uint16_t>(&ota_timer)->default_value(11), "Set OTA testing timer value in seconds (TDoOTA)")
    ...
    ("o",             bpo::value<string>(&args->mme_args.s1ap_args.output_directory_name)->default_value(""), "The name of the output directory")
  ...
  {
    if (args->mme_args.s1ap_args.output_directory_name == "") {
      fprintf(stderr, "Error: output directory name is empty. Please put --o option. \n");
      exit(1);
    }
  }
  ...
  args->mme_args.s1ap_args.ota_timer      = ota_timer;
  ...
  if (target_test_state == "Pre-AKA" || target_test_state == "PRE-AKA" || target_test_state == "pre-aka" || target_test_state == "0") {
    args->mme_args.s1ap_args.test_state = 0;
  } else if (target_test_state == "Post-AKA" || target_test_state == "POST-AKA" || target_test_state == "post-aka" || target_test_state == "1") {
    args->mme_args.s1ap_args.test_state = 1;
  } else if (target_test_state == "TAU" || target_test_state == "tau" || target_test_state == "Tau" || target_test_state == "2") {
    args->mme_args.s1ap_args.test_state = 2;
  } else if (target_test_state == "SR" || target_test_state == "Sr" || target_test_state == "sr" || target_test_state == "3") {
    args->mme_args.s1ap_args.test_state = 3;
    args->mme_args.s1ap_args.ota_timer = 4;
    cout << "Due to T3417: OTA timer is set to 4 seconds for SR testing." << endl;
  } else if (target_test_state == "DETACH" || target_test_state == "Detach" || target_test_state == "detach" || target_test_state == "4") {
    args->mme_args.s1ap_args.test_state = 4;
  } else if (target_test_state == "") {
    cout << "Error getting test_state. Please put --test_state option." << endl;
    exit(1);
  } else {
    cout << "Unsupported state:" << target_test_state << " - must be Pre-AKA, Post-AKA, TAU, SR or Detach." << endl;
    exit(1);
  }
