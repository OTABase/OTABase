...
#ifndef SRSEPC_NAS_H
#define SRSEPC_NAS_H
#define PRE_AKA_STATE 0 // same as EMM-REGISTERED-INITIATED
#define POST_AKA_STATE 1 // same as EMM-REGISTERED
#define TAU_STATE 2 // same as EMM-TRACKING-AREA-UPDATING-INITIATED
#define SR_STATE 3 // same as EMM-SERVICE-REQUEST-INITIATED
#define DETACH_STATE 4 // same as EMM-DEREGISTERED-INITIATED
...
typedef struct {
  ...
  uint16_t                            lac;
  /* OTAFUZZ*/
  bool                                reattach_enabled; 
  uint16_t                            test_state;    
  uint16_t                            ota_timer;
  bool                                replay_mode;
  bool                                blacklist_mode;
  bool                                reattach_measure;
} nas_init_t;
...
public:
  ...
  /* OTAFUZZ */
  bool handle_detach_accept(srsran::byte_buffer_t* nas_rx);
  ...
  bool pack_tracking_area_update_accept(srsran::byte_buffer_t* nas_buffer, LIBLTE_MME_EPS_UPDATE_TYPE_ENUM eps_update_type);
  /* OTAFUZZ */
  bool pack_test_ota_message(srsran::byte_buffer_t* nas_buffer, std::string payload, uint8 sec_hdr);
  bool pack_detach_request(srsran::byte_buffer_t* nas_buffer);
  bool pack_attach_reject(srsran::byte_buffer_t* nas_buffer);
  bool pack_service_accept(srsran::byte_buffer_t* nas_buffer);
  bool pack_detach_accept(srsran::byte_buffer_t* nas_buffer);
  ...
  bool                m_reattach_enabled = false;
  uint16_t            m_test_state = 0;
  bool                m_replay_mode = false;
  bool                m_blacklist_mode = false;
  LIBLTE_MME_EPS_UPDATE_TYPE_ENUM eps_update_type = LIBLTE_MME_EPS_UPDATE_TYPE_TA_UPDATING;
  bool                m_reattach_measure = false;

  bool is_guti_unknown_attach = false;
  bool once = true;
  bool send_next_nas_message = true;
  bool allow_detach_during_sr_procedure = false;
  bool is_guti_known_attach = false;
  bool handled_init_ctxt_resp = false;

  bool handle_uplink_detach_request = false;
...
private:
  ...
  uint16_t m_t3413 = 0;
  /* OTAFUZZ */
  uint16_t m_tdo_ota = 0;
  uint16_t m_tnas_oracle_timeout = 1;
  uint16_t m_treconn_timeout = 20; // tdo_ota+1
  ...
  bool start_tdo_ota();
  bool expire_tdo_ota();
  bool start_tnas_oracle_timeout();
  bool expire_tnas_oracle_timeout();
  bool start_treconn_timeout();
  bool expire_treconn_timeout();
};
