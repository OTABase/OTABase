...
/*********************************************************************
    ...
    Document Reference: 24.301 v10.2.0 Section 8.2.11.1
*********************************************************************/
...
/*********************************************************************
    Message Name: Detach Request (Net-initiated)

    Description: Sent by the network to request the release of an EMM
                 context.

    Document Reference: 24.301 v10.2.0 Section 8.2.11.2
*********************************************************************/
                                                         ...
                                                         LIBLTE_BYTE_MSG_STRUCT*                   msg)
{
  ...
  if (detach_req != NULL && msg != NULL) {
    ...
    *msg_ptr = 0;
    ...
    // EMM Cause
    if (detach_req->emm_cause_present) {
      *msg_ptr = LIBLTE_MME_EMM_CAUSE_IEI;
      msg_ptr++;
      liblte_mme_pack_emm_cause_ie(detach_req->emm_cause, &msg_ptr);
    }
...
/*********************************************************************
    ...
    Description: Sent by the network to the UE in order to accept the
                 service request procedure.

    Document Reference: 24.301 v10.2.0 Section 8.2.34
*********************************************************************/
                                                     ...
                                                     LIBLTE_BYTE_MSG_STRUCT*               msg)
{
  ...
  if (service_accept != NULL && msg != NULL) {
    ...
    *msg_ptr = LIBLTE_MME_MSG_TYPE_SERVICE_ACCEPT;
    ...
    // T3448 Value
    if (service_accept->t3448_present) {
      *msg_ptr = LIBLTE_MME_T3448_VALUE_IEI;
      msg_ptr++;
      liblte_mme_pack_gprs_timer_2_ie(service_accept->t3448, &msg_ptr);
    }
...
*********************************************************************/
LIBLTE_ERROR_ENUM
liblte_mme_unpack_tracking_area_update_request_msg(LIBLTE_BYTE_MSG_STRUCT*                            msg,
                                                  LIBLTE_MME_TRACKING_AREA_UPDATE_REQUEST_MSG_STRUCT* tau_req)
{
  ...
  if (msg != NULL && tau_req != NULL) {
    ...
    if ((LIBLTE_MME_NON_CURRENT_NATIVE_NAS_KEY_SET_IDENTIFIER_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_nas_key_set_id_ie(&msg_ptr, 0, &tau_req->non_curr_nas_ksi);
      msg_ptr++;
      tau_req->non_curr_nas_ksi_present = true;
    } else {
      tau_req->non_curr_nas_ksi_present = false;
    }

    // GPRS Ciphering Key Sequence Number
    if ((0x8 << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_ciphering_key_sequence_number_ie(&msg_ptr, 0, &tau_req->gprs_ciphering_key_seq_number);
      msg_ptr++;
      tau_req->gprs_ciphering_key_seq_number_present = true;
    } else {
      tau_req->gprs_ciphering_key_seq_number_present = false;
    }

    // Old P-TMSI Signature
    if (LIBLTE_MME_P_TMSI_SIGNATURE_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_p_tmsi_signature_ie(&msg_ptr, &tau_req->old_p_tmsi_signature);
      tau_req->old_p_tmsi_signature_present = true;
    } else {
    ...
    if (LIBLTE_MME_ADDITIONAL_GUTI_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_eps_mobile_id_ie(&msg_ptr, &tau_req->additional_guti);
      tau_req->additional_guti_present = true;
    } else {
    ...
    if (LIBLTE_MME_REPLAYED_NONCE_UE_IEI == *msg_ptr) {
      msg_ptr++;
      // liblte_mme_unpack_nonce_ue_ie(&msg_ptr, &tau_req->nonce_ue);
      //log 
      msg_ptr += 4;
      tau_req->nonce_ue_present = true;
    } else {
      tau_req->nonce_ue_present = false;
    }

    // UE Network Capability
    if (0x58 == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_ue_network_capability_ie(&msg_ptr, &tau_req->ue_network_cap);
      tau_req->ue_network_cap_present = true;
    } else {
      tau_req->ue_network_cap_present = false;
    }

    // Last Visited Registered TAI
    if (LIBLTE_MME_LAST_VISITED_REGISTERED_TAI_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_tracking_area_id_ie(&msg_ptr, &tau_req->last_visited_registered_tai);
      tau_req->last_visited_registered_tai_present = true;
    } else {
    ...
    if (LIBLTE_MME_DRX_PARAMETER_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_drx_parameter_ie(&msg_ptr, &tau_req->drx_param);
      tau_req->drx_param_present = true;
    } else {
      tau_req->drx_param_present = false;
    }

    // UE Radio Capability Information Update Needed
    if ((0xA << 4) == (*msg_ptr & 0xF0)) {
      // liblte_mme_unpack_ue_radio_capability_information_update_needed_ie(&msg_ptr,
      //                                                                     &tau_req->ue_radio_cap_info_update_needed);
      msg_ptr++;
      tau_req->ue_radio_capability_info_update_needed_present = true;
    } else {
      tau_req->ue_radio_capability_info_update_needed_present = false;
    }

    // EPS Bearer Context Status
    if (LIBLTE_MME_EPS_BEARER_CONTEXT_STATUS_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_eps_bearer_context_status_ie(&msg_ptr, &tau_req->eps_bearer_context_status);
      tau_req->eps_bearer_context_status_present = true;
    } else {
      tau_req->eps_bearer_context_status_present = false;
    }

    // MS Network Capability
    if (LIBLTE_MME_MS_NETWORK_CAPABILITY_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_ms_network_capability_ie(&msg_ptr, &tau_req->ms_network_cap);
      tau_req->ms_network_cap_present = true;
    } else {
      tau_req->ms_network_cap_present = false;
    }

    // Old Location Area ID
    if (LIBLTE_MME_LOCATION_AREA_IDENTIFICATION_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_location_area_id_ie(&msg_ptr, &tau_req->old_lai);
      tau_req->old_lai_present = true;
    } else {
    ...
    // TMSI Status
    if ((LIBLTE_MME_TMSI_STATUS_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_tmsi_status_ie(&msg_ptr, 0, &tau_req->tmsi_status);
      msg_ptr++;
    ...
    if (LIBLTE_MME_MS_CLASSMARK_2_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_mobile_station_classmark_2_ie(&msg_ptr, &tau_req->ms_cm2);
      tau_req->ms_cm2_present = true;
    } else {
    ...
    if (LIBLTE_MME_SUPPORTED_CODEC_LIST_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_supported_codec_list_ie(&msg_ptr, &tau_req->supported_codecs);
      tau_req->supported_codecs_present = true;
    } else {
    ...
    // Additional Update Type
    if ((LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_additional_update_type_ie(&msg_ptr, 0, &tau_req->additional_update_type);
      msg_ptr++;
      tau_req->additional_update_type_present = true;
    } else {
    ...
    if (LIBLTE_MME_VOICE_DOMAIN_PREF_AND_UE_USAGE_SETTING_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_voice_domain_pref_and_ue_usage_setting_ie(&msg_ptr,
                                                                  &tau_req->voice_domain_pref_and_ue_usage_setting);
      tau_req->voice_domain_pref_and_ue_usage_setting_present = true;
    } else {
    ...
    // Old GUTI Type
    if ((LIBLTE_MME_GUTI_TYPE_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_guti_type_ie(&msg_ptr, 0, &tau_req->old_guti_type);
      msg_ptr++;
    ...
    // Device Properties
    if ((0xD << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_device_properties_ie(&msg_ptr, 0, &tau_req->device_properties);
      msg_ptr++;
      tau_req->device_properties_present = true;
    } else {
...
LIBLTE_ERROR_ENUM liblte_mme_pack_ota_fuzz_emm_msg(std::string                           payload,
                                                   uint8                                 sec_hdr_type,
                                                   uint32                                count,
                                                   LIBLTE_BYTE_MSG_STRUCT*               msg)
{
  ...
  std::string payload_pd   = payload.substr(0,2);
  std::string payload_data = payload.substr(2);

  if (payload_pd != "07") {
    // return (err);
  }

  // Payload should be a complete plain NAS message. 
  // The length of NAS message is always even.
  if (payload != "" && msg != NULL) {
    if (LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS != sec_hdr_type) {
      // Protocol Discriminator and Security Header Type
      // Original code:
      // uint8_t pd_byte = static_cast<uint8_t>(std::strtoul(byte_str.c_str(), nullptr, 16)); // convert to byte value
      // uint8 pd_value = static_cast<uint8>(pd_str);

      *msg_ptr = (sec_hdr_type << 4) | (LIBLTE_MME_PD_EPS_MOBILITY_MANAGEMENT);
      msg_ptr++;

      // uint8_t byte = static_cast<uint8_t>(std::strtoul(byte_str.c_str(), nullptr, 16)); // convert to byte value
      // *msg_ptr = byte;
      // msg_ptr++;
    ...
    *msg_ptr = (LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS << 4) | (LIBLTE_MME_PD_EPS_MOBILITY_MANAGEMENT);
    ...
    for (int i = 0; i < payload_data.length(); i+=2){
      // Convert the pair of characters to a uint8_t value
      uint8_t byte_value = std::stoi(payload_data.substr(i,2), nullptr, 16);

      // Copy the byte to msg_ptr and increment the pointer
      *msg_ptr = byte_value;
      msg_ptr++;
    }
