...
bool pdcp_entity_base::integrity_verify(uint8_t* msg, uint32_t msg_len, uint32_t count, uint8_t* mac)
{
  ...
  //     for (int brid = 0; brid < 3; brid++){
  //       for (int dir = 0; dir < 2; dir++){
  //         if (dir == 0){
  //           cfg.rx_direction = SECURITY_DIRECTION_DOWNLINK;
  //         } else {
  //           cfg.rx_direction = SECURITY_DIRECTION_UPLINK;
  //         }
  //         security_128_eia2(&k_int[16], XX, brid - 1, cfg.rx_direction, msg, msg_len, mac_exp);
  //         logger.info("Integrity check input - COUNT %" PRIu32 ", Bearer ID %d, Direction %s",
  //                 XX,
  //                 brid,
  //                 cfg.rx_direction == SECURITY_DIRECTION_DOWNLINK ? "Downlink" : "Uplink");
  //         channel(mac_exp, 4, "MAC %s (expected):", is_valid ? "match" : "mismatch");
  //         if (mac[0] == mac_exp[0] && mac[1] == mac_exp[1] && mac[2] == mac_exp[2] && mac[3] == mac_exp[3]){
  //           logger.info("FOUND!!!!!!!!!!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
  //         }
  //       }
  //     }
