...
public:
  ...
  virtual bool     send_error_indication(nas* nas_ctx, uint16_t cause)                       = 0;
  ...
  virtual bool     send_downlink_nas_transport(uint32_t               enb_ue_s1ap_id,
                                               ...
                                               struct sctp_sndrcvinfo enb_sri)               = 0;
  /* OTAFUZZ */
  // Sends NAS test message got from python 
  virtual bool     send_nas_test_message_from_py(nas* nas_ctx)                               = 0;
  // Sends S-TMSI paging message
  virtual bool     send_paging_message(nas* nas_ctx)                                         = 0;
  // Handles the NAS Oracle
  virtual bool     handle_nas_oracle(nas* nas_ctx)                                           = 0;
};
