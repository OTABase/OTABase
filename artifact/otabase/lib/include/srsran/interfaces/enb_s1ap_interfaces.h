...
public:
  ...
  virtual void notify_nas_oracle(uint16_t rnti) = 0;
  ...
  /* OTAFUZZ */
  // Request nas message to EPC using s1ap message. 
  virtual bool send_nas_test_message(uint16_t rnti) = 0;
  // Request S-TMSI paging message to EPC using s1ap message.
  virtual bool send_paging_message(uint16_t rnti) = 0;
